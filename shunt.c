...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/11/27 22:40:55 by msafflow          #+#    #+#             */
/*   Updated: 2020/11/27 22:40:56 by msafflow         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 */
void push_numstackv(t_symtab_entry *val)
{
    if(nnumstack > MAXNUMSTACK-1)
    {
        fprintf(stderr, "error: Number stack overflow\n");
 ...
 */
t_symtab_entry *get_var(char *s, int *char_count)
{
    char *ss = s;
    if(*ss == '$')
    {
    ...
    t_symtab_entry *e = get_symtab_entry(name);
    if(!e)
    {
    ...
    for( ; *expr; )
    {
       if(!tstart)
        ...
        else
        {
            ...
            {
                t_symtab_entry *n1 = get_var(tstart, &n2);
                if(!n1)
                {
