...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/11/26 18:42:53 by msafflow          #+#    #+#             */
/*   Updated: 2020/11/26 23:07:20 by msafflow         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
** and start with an alphabetic char or underscore.
...
char				*substitute_str(char *s1, char *s2, size_t start, size_t end)
{
	...
	before[start] = '\0';
	size_t afterlen = ft_strlen(s1) - end + 1;
	char after[afterlen];
	strcpy(after, s1 + end + 1);
	size_t totallen = start + afterlen + ft_strlen(s2);
...
t_word *word_expand(char *orig_word)
{
	...
	char *pstart = malloc(ft_strlen(orig_word) + 1);
	if (!pstart)
		return (NULL);
